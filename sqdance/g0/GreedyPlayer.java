...
public class GreedyPlayer implements sqdance.sim.Player {
	...
	public Point[] play(Point[] dancers, int[] scores, int[] partner_ids, int[] enjoyment_gained) {
		...
		for (int i = 0; i < d; i++) {
			...
			Point m = null;
			if (++idle_turns[i] > 21) { // if stuck at current position without
										// enjoying anything
				idle_turns[i] = 0;
			} else { // stay put if there's another potential dance partner in
						// range
